...
const CircleLayout = ({ numItems }: { numItems: number }) => {
    const radius = 13; // 半径，决定圆的大小
    const angleStep = 360 / numItems; // 每个子元素的角度步长
    const [items, setItems] = useState(Array.from({ length: numItems }).map(() => ({ x: 0, y: 0 })))
    ...
    return (
        <div className={styles.container}>
            <div className={styles.element}>
                ...
                <div className={styles.box} style={{ width: '6%', bottom: '-9vw', right: '12vw' }} />
                <div className={styles.box} style={{ width: '8%', bottom: '-10vw', right: '15vw' }} />
            </div>
            {items.map((item, index) => (
                ...
                >
                    <strong>99</strong>
                    <div className={styles.name}>А5.5.3.4090</div>
                </div>
            ))}
        </div>
    );
